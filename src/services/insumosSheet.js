...
async function exportarTablas() {
  ...
  for (const [tabla, descripcion, clave] of TABLAS) {
    ...
    filas.sort((a, b) => { const ka = clave(a); const kb = clave(b); for (let i = 0; i < ka.length; i += 1) { const c = cmp(ka[i], kb[i]); if (c) return c; } return 0; });
